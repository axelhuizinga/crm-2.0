...
@:connect
class DevForm extends ReactComponentOf<DevProps, UserState>
{
	var submitValue:String;
	static var uBCC:Dynamic;
	static var uBState:Dynamic;

	public function new(?props:DevProps)
	{
	...
	override public function shouldComponentUpdate(nextProps:DevProps, nextState:UserState) {
	...
	static function mapStateToProps() {
		...
		return function(aState:AppState):Partial<DevProps>
		{
	...
	{
		//trace(Reflect.fields(props));
		//trace(state.waiting);${renderForm()}	
		var style = 
		{
			maxWidth:'32rem'
		};
